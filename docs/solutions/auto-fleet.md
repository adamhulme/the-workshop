...
- **SHA-256 hash check on manifest.** Captured once at fleet start; verified once at step 8 before writing on the fleet branch. (The original draft re-checked per iteration, but the round-1 fix moved all state in-memory during the dispatch loop — the manifest doesn't exist on the `auto-do/<id>` working tree, so per-iteration re-reads would always fail. Single check at step 8 is the only correct point.) External edits halt cleanly with `Final status: halted:manifest-tampered` rather than silently clobbering user edits.
...
- **Outcome classification keyed off explicit strings.** `/auto-do`'s actual `Final status:` contract tokens (`success`, `failed:round-2-must-fix`, `failed:test-gate`, `failed:complexity-smell`, `failed:ambiguity` — see `commands/auto-do.md:212`) drive the row state. Note the rename asymmetry: `/auto-do` emits `success` (an existing contract); `/auto-fleet` translates that to manifest row state `succeeded` (industry convention). Anything unrecognised → `failed` with `Final status: halted:unrecognised-auto-do-report`.
- **State names follow industry convention.** Manifest row states: `queued | running | succeeded | failed | skipped` — matches Argo Workflows / Temporal / GitHub Actions; `succeeded` pairs with `failed` more cleanly than the original `done`. (The `/auto-do` report token stays `success` per its existing contract; the rename is to the manifest schema only.)
