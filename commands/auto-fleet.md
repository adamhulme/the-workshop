...
- **Verify the fleet branch is rooted on `<default>` and contains only manifest commits.** Run `git diff <default>..HEAD --name-only`. The set of changed paths must be empty (branch is at `<default>`) or exactly `{docs/fleet/<slug>.md}`. If any other path appears, bail with: "Branch `fleet/<slug>` has commits ahead of `<default>` that touch files other than `docs/fleet/<slug>.md`: `<offending-paths>`. The control-plane branch must be branched from `<default>` and contain only manifest commits. Re-create with `git checkout -b fleet/<slug> <default>` and re-author the manifest." (The branch can — and usually will — be ahead of `<default>` by one or more commits that authored the manifest; that's expected. The guard rejects only off-purpose commits.)
...
4. **Outcome classification.** Read `/auto-do`'s final report. Match its `Final status:` line (`/auto-do`'s contract; see `commands/auto-do.md:212`):
   ...
   - Anything else, or the `Final status:` line missing → row state `failed`. Fleet `Final status: halted:unrecognised-auto-do-report`. **Capture the first 200 chars of `/auto-do`'s actual report into a `### Unrecognised /auto-do report — <id>` subsection of the `## Fleet outcome` section at step 8** (the manifest schema has no per-row `note` column; unstructured diagnostics belong in the outcome section, not the table).
...
4. **Compose the final manifest in memory:**
   ...
   - Append a `## Fleet outcome` section with:
     ...
     - `Final status:` exactly one of `succeeded` (no `failed` rows and no `queued`-remaining — i.e. every row reached a terminal non-failure state, `succeeded` or `skipped`), `halted:round-2-failure`, `halted:test-gate`, `halted:complexity-smell`, `halted:auto-do-ambiguity`, `halted:unrecognised-auto-do-report`, `halted:manifest-tampered`, `halted:branch-collision-cancel`, `halted:user-cancel`. A fleet of all-`skipped` rows (every row hit the idempotency-gate `Skip` recommendation) is `succeeded` — the recommended-skip path is a normal happy path, not a failure.
